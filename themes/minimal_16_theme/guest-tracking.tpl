...
{if isset($order_collection)}
	...
	{if isset($transformSuccess)}
		<p class="alert alert-success"><span class="alert-content">{l s='Your guest account has been successfully transformed into a customer account. You can now login as a registered shopper. '} <a href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}">{l s='page.'}</a></span></p>
	{else}
