        ...
        {foreach from=$languages item=lang}
            <div class="tab-pane {if $lang.id_lang == $current_id_lang}active{/if}" id="lang{$lang.id_lang}">
                <div class="form-group">
                    ...
                    <input class="is_required validate form-control" data-validate="isGenericName" type="text" id="product_name" name="product_name[{$lang.id_lang}]" required="" value="{if isset($product['name'][$lang.id_lang])}{$product['name'][$lang.id_lang]|escape:'html':'UTF-8'}{/if}"/>
                </div>
                <div class="form-group">
                    <label class="required" for="product_short_description">{l s='Short Description' mod='npsmarketplace'}</label>
                    <textarea class="is_required validate form-control" data-validate="isMessage" id="product_short_description" name="product_short_description[{$lang.id_lang}]" rows="2">{if isset($product['description_short'][$lang.id_lang])}{$product['description_short'][$lang.id_lang]|escape:'html':'UTF-8'}{/if}</textarea>
                </div>
                <div class="form-group">
                    <label class="required" for="product_description">{l s='Description' mod='npsmarketplace'}</label>
                    <textarea class="is_required validate form-control" data-validate="isMessage" id="product_description" name="product_description[{$lang.id_lang}]" rows="10">{if isset($product['description'][$lang.id_lang])}{$product['description'][$lang.id_lang]|escape:'html':'UTF-8'}{/if}</textarea>
                </div>
            </div>
        {/foreach}
    </div>
    ...
    <div class="row">
        <div class="form-group col-md-6">
            ...
            <div id="datePicker" class="input-append">
                <input class="is_required form-control" id="product_date" name="product_date" data-format="yyyy-MM-dd" type="text" readonly="" required="" value="{if isset($product['available_date'])}{$product['available_date']|escape:'html':'UTF-8'}{/if}"/>
        ...
        <div class="form-group col-md-6">
            ...
            <div id="timePicker" class="input-append">
                <input class="is_required form-control" id="product_time" name="product_time" data-format="hh:mm" type="text" readonly="" value="{if isset($product['available_time'])}{$product['available_time']|escape:'html':'UTF-8'}{/if}"/>
                <span class="add-on"> <i data-time-icon="icon-time" data-date-icon="icon-calendar"></i> </span>
            </div>
        </div>
    </div>
