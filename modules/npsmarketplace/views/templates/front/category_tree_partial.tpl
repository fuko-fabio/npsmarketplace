<li class="category_{$node.id}{if isset($last) && $last == 'true'} last{/if}">
    <p class="checkbox">
        <input type="checkbox" name="category[]" id="category_{$node.id}" value="{$node.id}" {if in_array($node.id, $product['categories'])}checked=""{/if} title="{$node.desc|strip_tags|trim|truncate:255:'...'|escape:'html':'UTF-8'}"/>
