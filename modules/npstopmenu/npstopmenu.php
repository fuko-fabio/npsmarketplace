...
require (dirname(__FILE__).'/npsmenutoplinks.class.php');

class npstopmenu extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('nps Top horizontal menu');
	...
	public function install()
	{
		if (!parent::install() ||
			...
			!Configuration::updateGlobalValue('MOD_NPSTOPMENU_ITEMS', 'CAT3,CAT26') ||
			!Configuration::updateGlobalValue('MOD_NPSTOPMENU_SEARCH', '1') ||
	...
	public function installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'npslinksmenutop` (
			`id_npslinksmenutop` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
			Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'npslinksmenutop_lang` (
			`id_npslinksmenutop` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_npslinksmenutop` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;'));
	}
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('MOD_NPSKTOPMENU_ITEMS') ||
			!Configuration::deleteByName('MOD_NPSTOPMENU_SEARCH') ||
			!$this->uninstallDB())
			return false;
		return true;
	}
	...
	{
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'npslinksmenutop`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'npslinksmenutop_lang`');
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitNpstopmenu'))
		{
			$items = Tools::getValue('items');
			if (is_array($items) && count($items))
 				$updated = Configuration::updateValue('MOD_NPSKTOPMENU_ITEMS', (string)implode(',', $items));
 			else
 				$updated = Configuration::updateValue('MOD_NPSTOPMENU_ITEMS', '');
 			if ($updated)
		...
		else if (Tools::isSubmit('submitNpstopmenuLinks'))
		{
			...
			if ($count_links_label || $count_label)
			{
				...
				{
					NpsMenuTopLinks::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
					$this->_html .= $this->displayConfirmation($this->l('The link has been added.'));
				}
			}
			$update_cache = true;
		}
		elseif (Tools::isSubmit('deletenpslinksmenutop'))
		{
			$id_npslinksmenutop = Tools::getValue('id_npslinksmenutop', 0);
			NpsMenuTopLinks::remove($id_npslinksmenutop, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_NPSKTOPMENU_ITEMS', str_replace(array('LNK'.$id_npslinksmenutop.',', 'LNK'.$id_npslinksmenutop), '', Configuration::get('MOD_NPSTOPMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed.'));
			$update_cache = true;
		}
		elseif (Tools::isSubmit('updatenpslinksmenutop'))
		{
			$id_npslinksmenutop = (int)Tools::getValue('id_npslinksmenutop', 0);
			...
			if (Tools::isSubmit('updatelink'))
			{
				...
				NpsMenuTopLinks::update($link, $label, $new_window, (int)$id_shop, (int)$id_npslinksmenutop, (int)$id_npslinksmenutop);
		...
		$links = NpsMenuTopLinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
	...
	private function getMenuItems()
	{
		...
		{
			$conf = Configuration::get('MOD_NPSKTOPMENU_ITEMS');
			if (strlen($conf))
				return explode(',', Configuration::get('MOD_NPSKTOPMENU_ITEMS'));
			else
				return array();
		}
	}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = NpsMenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = NpsMenuTopLinks::get($link[0]['id_npslinksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
						$html .= '<option selected="selected" value="LNK'.(int)$link[0]['id_npslinksmenutop'].'">'.Tools::safeOutput($link[0]['label']).'</option>';
					}
					break;
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = NpsMenuTopLinks::get($link[0]['id_npslinksmenutop'], $default_language, (int)Shop::getContextShopID());
						}
	...
	protected function getCacheId($name = null)
	{
		...
		return 'npstopmenu|'.(int)Tools::usingSecureMode().'|'.$page_name.'|'.(int)$this->context->shop->id.'|'.implode(', ',$this->user_groups).'|'.(int)$this->context->language->id.'|'.(int)Tools::getValue('id_category').'|'.(int)Tools::getValue('id_manufacturer').'|'.(int)Tools::getValue('id_supplier').'|'.(int)Tools::getValue('id_cms').'|'.(int)Tools::getValue('id_product');
	}
	...
	public function hookDisplayTop($param)
	{
		...
		if (!$this->isCached('npstopmenu.tpl', $this->getCacheId()))
		{
			if (Tools::isEmpty($this->_menu))
				$this->makeMenu();
			$this->smarty->assign('MENU_SEARCH', Configuration::get('MOD_NPSTOPMENU_SEARCH'));
		...
		$this->context->controller->addJS($this->_path.'js/npstopmenu.js');
		$this->context->controller->addCSS($this->_path.'css/npstopmenu.css');
		$this->context->controller->addCSS($this->_path.'css/superfish-modified.css');

		$html = $this->display(__FILE__, 'npstopmenu.tpl', $this->getCacheId());
		return $html;
	}
	...
	{
		$this->_clearCache('npstopmenu.tpl');
	}
	
	public function hookActionShopDataDuplication($params)
	{
		$npslinksmenutop = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'npslinksmenutop 
			WHERE id_shop = '.(int)$params['old_id_shop']
			);

		foreach($npslinksmenutop as $id => $link)
		{
			Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'npslinksmenutop (id_npslinksmenutop, id_shop, new_window) 
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');
			
			$npslinksmenutop[$id]['new_id_npslinksmenutop'] = Db::getInstance()->Insert_ID();
		}
		...
		foreach($npslinksmenutop as $id => $link)
		{
			$lang = Db::getInstance()->executeS('
					...
					FROM '._DB_PREFIX_.'npslinksmenutop_lang 
					WHERE id_npslinksmenutop = '.(int)$link['id_npslinksmenutop'].' AND id_shop = '.(int)$params['old_id_shop']);
			
			foreach($lang as $l)
				Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'npslinksmenutop_lang (id_npslinksmenutop, id_lang, id_shop, label, link) 
					VALUES ('.(int)$link['new_id_npslinksmenutop'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
		}
	}
	...
	public function renderAddForm()
	{
		$fields_form = array(
			'form' => array(
				'legend' => array(
					'title' => (Tools::getIsset('updatenpslinksmenutop') && !Tools::getValue('updatenpslinksmenutop')) ? $this->l('Update link') : $this->l('Add a new link'),
					'icon' => 'icon-link'
				),
				'input' => array(
					array(
		...
		if (Tools::getIsset('updatenpslinksmenutop') && !Tools::getValue('updatenpslinksmenutop'))
			$fields_form['form']['submit'] = array(
				'name' => 'updatenpslinksmenutop',
				'title' => $this->l('Update')
			);

		if (Tools::isSubmit('updatenpslinksmenutop'))
		{			
			$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'updatelink');
			$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_npslinksmenutop');			
	...
	public function renderChoicesSelect()
	{
		...
		$links = NpsMenuTopLinks::gets($this->context->language->id, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$default_language = Configuration::get('PS_LANG_DEFAULT');
				$link = NpsMenuTopLinks::get($link['id_npslinksmenutop'], $default_language, (int)Shop::getContextShopID());
				if (!in_array('LNK'.(int)$link[0]['id_npslinksmenutop'], $items))
					$html .= '<option value="LNK'.(int)$link[0]['id_npslinksmenutop'].'">'.$spacer.Tools::safeOutput($link[0]['label']).'</option>';
			}
			elseif (!in_array('LNK'.(int)$link['id_npslinksmenutop'], $items))
				$html .= '<option value="LNK'.(int)$link['id_npslinksmenutop'].'">'.$spacer.Tools::safeOutput($link['label']).'</option>';
		}
		$html .= '</optgroup>';
		$html .= '</select>';
		return $html;
	...
	{
		return array(
			'search' => Tools::getValue('search', Configuration::get('MOD_NPSTOPMENU_SEARCH'))
		);
	}
	...
	public function getAddLinkFieldsValues()
	{
		...
		if (Tools::isSubmit('updatenpslinksmenutop'))
		{
			$link = NpsMenuTopLinks::getLinkLang(Tools::getValue('id_npslinksmenutop'), (int)Shop::getContextShopID());
		...
		$fields_values = array(
			...
			'id_npslinksmenutop' => Tools::getValue('id_npslinksmenutop'),
		);
	...
	{
		$links = NpsMenuTopLinks::gets((int)$this->context->language->id, null, (int)Shop::getContextShopID());
		$fields_list = array(
			'id_npslinksmenutop' => array(
		...
		$helper->identifier = 'id_npslinksmenutop';
		$helper->table = 'npslinksmenutop';
