...
<div id="new-products_block_right" class="block products_block">
	<h4 class="title_block"><a href="{$link->getPageLink('new-products')|escape:'html'}" title="{l s='New products' mod='npsnewproducts'}">{l s='New products' mod='npsnewproducts'}</a></h4>
	...
	{if $new_products !== false}
		...
		{foreach from=$new_products item=newproduct name=myLoop}
			...
			{if $newproduct.description_short}<dd class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}"><a href="{$newproduct.link|escape:'html'}">{$newproduct.description_short|strip_tags:'UTF-8'|truncate:75:'...'}</a><br /><a href="{$newproduct.link}" class="lnk_more">{l s='Read more' mod='npsnewproducts'}</a></dd>{/if}
		{/foreach}
		</dl>
		<p><a href="{$link->getPageLink('new-products')|escape:'html'}" title="{l s='All new products' mod='npsnewproducts'}" class="button_large">&raquo; {l s='All new products' mod='npsnewproducts'}</a></p>
	{else}
		<p>&raquo; {l s='Do not allow new products at this time.' mod='npsnewproducts'}</p>
	{/if}
	</div>
</div>
<!-- /MODULE Block new products -->
